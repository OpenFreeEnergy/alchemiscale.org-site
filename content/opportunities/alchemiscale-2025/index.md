...
Originally concieved as a wrapper around [Folding@Home](https://foldingathome.org/) for performing alchemical binding free energy calculations routinely and at scale, the project co-evolved with the [**OpenFE**](https://openfree.energy/) data model to become a general-purpose, high-throughput execution system that can take advantage of a variety of distributed compute.
...
- [0.3.0](https://github.com/OpenFreeEnergy/alchemiscale/releases/tag/v0.3.0) : improvements to users’ ability to influence compute effort allocation, support for large `AlchemicalNetwork`s
    ...
    - vast speed improvement to `AlchemicalNetwork` submission and retrieval
- [0.4.0](https://github.com/OpenFreeEnergy/alchemiscale/releases/tag/v0.4.0) : performance improvements, both user- and compute-facing
    ...
    - added concept of `AlchemicalNetwork` *state*, allowing users to set them `inactive`, `deleted`, or `invalid` when no longer relevant
...
This will allow users to optionally associate a parameterized `Strategy` with the `AlchemicalNetwork`s they submit, and this `Strategy` will be applied periodically server-side to adaptively sample `Transformation`s until the `Strategy`'s stopping thresholds are satisfied.
...
- [0.6.0](https://github.com/OpenFreeEnergy/alchemiscale/milestone/16) : result retrieval optimizations, server-side Task restart policies
    ...
    - `AlchemiscaleClient` persistent caching for `ProtocolDAGResult`s
...
- using utilization-based metrics to spin up and down compute services as one could do on e.g. Kubernetes via a `HorizontalPodAutoscaler` is too blunt an instrument: different `Protocol`s will utilize the GPU and CPU resources differently, making thresholds for scaling up and down difficult to set, and when resources are scaled down no distinction is made between those doing work and those idling, wasting potentially hours of expensive GPU compute each from indiscriminate deletion of running containers
...
However, there are areas where additional effort could enable more flexibility for users using `alchemiscale` in this way.
...
A compute service that instead performed several `ProtocolUnit`s of a `ProtocolDAG` in parallel could make better use of the resources it is deployed on, saturating its available GPU(s) and CPU(s).
Such a compute service could also be written to perform *multiple* `ProtocolDAG`s at once, which could work to ensure high GPU saturation for `ProtocolDAG`s with low intrinsic parallelism.
...
A relatively easy way to avoid this is for the compute service to periodically check that its claimed `Task`s are still actioned *somewhere*, and if they are not, halt execution immediately and drop the claims.
...
Compute services consume work from the set of all actioned and `waiting` `Task`s within their visible `Scope`s.
...
These `Strategy`s would then each propose on which `Transformation`s effort should be applied next given current results, and these proposals will be aggregated to form the overall proposal on which action is taken.
The `Strategy`s effectively function as a committee, and in a way "vote" on what should be done next.

One scenario in which this could work well: stacking two `Strategies`, where one is more opinionated when few results exist and the other when many results exist.
